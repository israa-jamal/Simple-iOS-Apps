...
struct DestiniBrain {
    //array of all the posabilites in the story
    //choice destination refers to the index of the story in the array
    let storyLine = [
    ...
    var distIndex = 0 //which line we are in currently
    
    func getStory() -> String{
        return storyLine[distIndex].title
    }
    mutating func checkChoice(_ userChoice: String){
        if userChoice == storyLine[distIndex].choice1
        {
            distIndex = storyLine[distIndex].choice1Destination
        }
        else{
            distIndex = storyLine[distIndex].choice2Destination
        }
    }
    
    func getChoice1() -> String{
        return storyLine[distIndex].choice1
    }
    func getChoice2() -> String{
        return storyLine[distIndex].choice2
    }
